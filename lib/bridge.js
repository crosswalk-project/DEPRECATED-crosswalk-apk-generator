...
var construct = function () {
   ...
   * {String[]} args Array of arguments to pass to the script (optional)
   ...
   *
   * cb(err, stdout, stderr); note that this has a different
   * signature from other parts of the API as this method is only
   ...
   *
   * cb(err, result); if subcommand is 'debug', result will match
   * /PORT (\d+)/, where the captured part of the regex is the remote
   * port number
   */
...
module.exports = {
   ...
   * config.tizenAppScriptPath: (optional) Path to tizen-app.sh on the device;
